...
export function canonicalizeItems(items: ChecklistItemConfig[]): string {
...
// Synchronous SHA-256 implementation in pure JS to guarantee identical results anywhere
export function sha256Hex(ascii: string): string {
  ...
  for (j = 0; j < words[lengthProperty];) {
    ...
    for (i = 0; i < 64; i++) {
      ...
      const temp1 = (hash[7] + s1H + ch + k[i] + (w[i] = (i < 16) ? w[i] : (w[i - 16] + s0 + w[i - 7] + s1) | 0)) | 0;
...
export function deriveChecklistId(items: ChecklistItemConfig[]): string {
